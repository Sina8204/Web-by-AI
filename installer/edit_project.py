...
from operator_and_waiting_class import ChatWindow

py_3_9 = r"D:\Apps\Python\installed\3.9\python.exe"
borders_relief="solid"
...
class project_manager:
    ...
    def on_item_double_click(self, event):
        ...
        path = []
        while selected_item:
            path.insert(0, self.tree.item(selected_item, "text"))  # نام آیتم
            selected_item = self.tree.parent(selected_item)  # والد آیتم
        if path[0] == "Root":
            path.pop(0)
        item_text = "/".join(path)
        print(item_text)
        if item_text:
            ...
            path = f"{fe.open_temp_file()["project_path"]}/{item_text}"
            print(f"path : {path}")
...
class edit_project_class:
    def __init__(self , root):
        ...
        self.pw_top_horizontala = tk.PanedWindow(self.vertical_pw , orient=tk.HORIZONTAL, bd=1 , relief=borders_relief)
        self.pw_down_horizontala = tk.PanedWindow(self.vertical_pw , orient=tk.HORIZONTAL, bd=1 , relief=borders_relief)
        ...
        self.cod_pw = tk.PanedWindow(self.pw_top_horizontala , bd=1 , relief=borders_relief)
        # self.web_pw = tk.PanedWindow(self.pw_top_horizontala)
        self.operator_pw = tk.PanedWindow(self.pw_top_horizontala , bd=1 , relief=borders_relief)
        self.pw_top_horizontala.add(self.cod_pw)
        self.pw_top_horizontala.add(self.operator_pw)
        # self.pw_top_horizontala.add(self.web_pw)

        self.file_manager_pw = tk.PanedWindow(self.pw_down_horizontala , bd=1 , relief=borders_relief)
        self.ai_editor_pw = tk.PanedWindow(self.pw_down_horizontala , bd=1 , relief=borders_relief)
        self.pw_down_horizontala.add(self.file_manager_pw)
        self.pw_down_horizontala.add(self.ai_editor_pw)
        
        #++++++++++++++++++++++++ <Frames> #++++++++++++++++++++++++
        self.fram_cod = tk.Frame(self.cod_pw)
        # self.fram_web = tk.Frame(self.web_pw)
        self.fram_operator = tk.Frame(self.operator_pw)
        self.fram_file_manager = tk.Frame(self.file_manager_pw , width=300)
        self.fram_ai_editor = tk.Frame(self.ai_editor_pw)

        self.cod_pw.add(self.fram_cod, minsize=800)   # حداقل ارتفاع یا عرض
        self.file_manager_pw.add(self.fram_file_manager, minsize=300)
        # self.web_pw.add(self.fram_web)
        self.operator_pw.add(self.fram_operator)
        self.ai_editor_pw.add(self.fram_ai_editor)

        self.codes_and_scripts = codes_and_scripts(self.fram_cod)
        self.operator = ChatWindow(self.fram_operator)
