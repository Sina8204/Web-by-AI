        ...
        if path:
            if os.path.exists(path):
                if path.lower().endswith(".html"):
                    ...
                    messagebox.showerror(title = "Error !!!" , message="pleas run just .html files")
            else :
                messagebox.showerror(title = "Error !!!" , message=f"There isn't any file in '{path}' directory\nif you change the path or directory of project , you should change theme in the details.json file")
    ...
    def open_with_other_browser(self , browser_path , html_path):
        if browser_path and os.path.exists(browser_path): 
            if html_path and os.path.exists(html_path): 
                ...
                messagebox.showerror(title="Error" , message= f"Invalid html path '{html_path}'\nPleas enter html file path")
        else:
            messagebox.showerror(title="Error" , message= f"Invalid browser path '{browser_path}'\nPleas enter true browser path")
